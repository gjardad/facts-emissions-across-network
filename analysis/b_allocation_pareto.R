###############################################################################
# analysis/b_allocation_pareto.R
#
# PURPOSE
#   Assign scope 1 emissions to all Belgian firms for each year and each of
#   B subsample draws, using Pareto (GPA) redistribution.
#
#   This is the SINGLE SOURCE OF TRUTH for firm-level emission assignment.
#   Downstream scripts (scope 1 dispersion, upstream emissions, etc.) load
#   the output and never recompute the allocation.
#
...
#   {PROC_DATA}/deployment_draw_params.RData  (optional; fallback if absent)
#   {REPO_DIR}/preprocess/crosswalks/nace_crf_crosswalk.csv
#
# OUTPUT
#   {PROC_DATA}/allocation_pareto/alloc_YYYY.RData  (one file per year)
...
OUT_DIR <- file.path(PROC_DATA, "allocation_pareto")
if (!dir.exists(OUT_DIR)) dir.create(OUT_DIR, recursive = TRUE)

cat("===================================================================\n")
cat("  PARETO EMISSION ALLOCATION (single source of truth)\n")
...
# =============================================================================
# SECTION 4: GPA + threshold -- draw-specific or global fallback
# =============================================================================
...
for (t in YEARS) {
  ...
  # ETS emitters with CRF group assignment (for combined ranking)
